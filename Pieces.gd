...
# FIRE
func fire_illegal_actions() -> Array:
	var illegal_actions := []
	for x in range(Board.width):
		for y in range(Board.height):
			var v = Vector2i(x, y)
			if !(Board.is_free(v) or Board.is_frozen(v)):
				illegal_actions.append(v)
	return illegal_actions

func fire_act(from, to):
	if Board.get_piece(to) != -1:
		Board.set_frost(to, 0)
	else:
		Board.set_ice(to, Board.IceType.Water)


# WATER
func water_illegal_actions() -> Array:
	var illegal_actions := []
	for x in range(Board.width):
		for y in range(Board.height):
			var v = Vector2i(x, y)
			if (Board.get_ice(v) != Board.IceType.Ice and
				(!Board.is_frozen(v) or Board.get_piece(v) == Board.PieceType.Reactor)):
				illegal_actions.append(v)
	return illegal_actions

func water_act(from, to):
	if Board.get_piece(to) != -1:
