...
func generate_rocks() -> void:
	...
	for i in range(4):
		...
		IceTM.set_cell(Vector2i(x, y), IceType.Rock, Vector2i(randi_range(0, 2), 0))
		IceTM.set_cell(Vector2i(width - x - 1, height - y - 1), IceType.Rock, Vector2i(randi_range(0, 2), 0))
		...
		for y in range(height):
			...
			for x2 in range(-2, 2):
				for y2 in range(-2, 2):
					...
					if get_piece(v2) != -1: set_frost(v2, 0)
